...
- (NSString *)publisherIdForAd:(AdMobView *)adView {
  return @"a14d6d30d9287db"; // this should be prefilled; if not, get it from www.admob.com
}
...
- (UIColor *)adBackgroundColorForAd:(AdMobView *)adView {
  return [UIColor colorWithRed:0.0 green:0.0 blue:0.0 alpha:0.90]; // this should be prefilled; if not, provide a UIColor
}
...
- (void)didFailToReceiveAd:(AdMobView *)adView {
    ...
    //So instead, let's present our own !
    //UIImage* igoAdImage = [UIImage imageNamed:@"iGoProAd"];
    
    //UIImageView *igoAdImageView = [[UIImageView alloc] initWithImage:igoAdImage];
    //igoAdImageView.frame = self.view.frame;
    //[self.view addSubview:igoAdImageView];
    //NSLog(@"Self View : %@",self.view);
    //[igoAdImageView release];
}
