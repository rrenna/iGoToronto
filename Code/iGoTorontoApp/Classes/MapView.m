...
- (id) initWithFrame:(CGRect) frame
{
	...
	if (self != nil) {
		self.mapView = [[[MKMapView alloc] initWithFrame:CGRectMake(0, 0, frame.size.width, frame.size.height)] autorelease];
		self.mapView.showsUserLocation = YES;
		[self.mapView setDelegate:self];
		[self addSubview:self.mapView];
		routeView = [[UIImageView alloc] initWithFrame:CGRectMake(0, 0, self.mapView.frame.size.width, self.mapView.frame.size.height)];
...
-(NSArray*) calculateRoutesFrom:(CLLocationCoordinate2D) f to: (CLLocationCoordinate2D) t {
	...
	if([components count] > 0)
	{
		...
		NSArray* routeDetailsComponents = [[routeDescriptionDetails stringByReplacingOccurrencesOfString:@"\x26#160;" withString:@""] componentsSeparatedByString:@"/"];
		NSLog(@"%@",routeDescriptionDetails);
	}*/
